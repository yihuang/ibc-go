...
func (suite *InterchainAccountsTestSuite) TestOnChanOpenInit() {
	...
	testCases := []struct {
		...
		{
			"ICA OnChanOpenInit fails with ErrInvalidChannelFlow", func() {}, false,
		},
	}
...
func (suite *InterchainAccountsTestSuite) TestOnChanOpenAck() {
	...
	testCases := []struct {
		...
		{
			"ICA OnChanOpenAck fails with ErrInvalidChannelFlow", func() {}, false,
		},
	}
	...
	for _, tc := range testCases {
		...
		suite.Run(tc.name, func() {
			...
			err = cbs.OnChanOpenAck(suite.chainA.GetContext(), path.EndpointA.ChannelConfig.PortID, path.EndpointA.ChannelID, path.EndpointA.ChannelConfig.Version)
